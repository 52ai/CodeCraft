...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num, char * filename){
    ...
    **/
    ...
    int aver_best[graph_node_num];//记录粒子群的平均最好位置

    int optimal_repeat_cnt=0;//重复最优解的次数
    double QBPSO_alpha = 1.4;//QBSP算法中的收缩扩张系数

    srand((unsigned)time(0));

    for(int i=0;i<Swarm_Size;i++ ){
        for(int j=0;j<graph_node_num;j++){//随机产生个体
            if(rand()/(RAND_MAX+1.0) > 0.7) Swarm[i].push_back(1);
            else Swarm[i].push_back(0);

            p_best_array[i].push_back(Swarm[i][j]);//初始化个体最好位置
            cout << Swarm[i][j];
         }
        cout << endl;
    }
    ...
    g_best = INF;
    g_best_id = Swarm_Size-1;//初始化当前全局最好位置
    /*
    ...
    while(((finish_time - start_time)/CLOCKS_PER_SEC < 70) && optimal_repeat_cnt < 20){//每一次循环进行一次粒子的更新并计算适应度
        //QBPSO 求解当前粒子群平均最好位置，每次得到一个最新的平均组号位置
        for(int j=0;j<graph_node_num;j++){

            int sum =0;
            for(int i=0;i<Swarm_Size;i++){
                sum += p_best_array[i][j];
            }

            double avg = sum/(Swarm_Size+1.0);
            printf("sum:%d,avg:%f\n",sum,avg);
            if( avg > 0.5) aver_best[j] = 1;
            else  aver_best[j] =0;
        }
        for(int j=0;j<graph_node_num;j++){
                cout<<aver_best[j];
        ...
        for(int i=0;i<Swarm_Size;i++){
            ...
            cout <<"第"<<i+1<<"个粒子："<<"可提供流量"<< mcmf->flow << ",当前总的费用："<<last_cost<<"，该粒子历史最小费用:"<<p_best[i]<<",全局最小费用："<<g_best<<endl;
            delete mcmf;//回收内存
        }

        int old_g_best = g_best;
        //QBPSO 求得当前全局最优值(基于选择操作获得全局最优解)
        
        int k = rand()%Swarm_Size;//在粒子群中随机选择一个粒子k
        if(p_best[k] < g_best){
            g_best_id = k;
            g_best = p_best[k];
        }
        ...
        for(int i=0;i<Swarm_Size;i++){//遍历粒子个体

            //QBPSO step 5 对每一个粒子求pi(局部吸引子) 可通过类似遗传算法的交叉操作获得
            //即通过个体最优p_best和种群最优g_best交叉操作获得
            vector<int> p1_gen;//子代粒子p1
            vector<int> p2_gen;//子代粒子p2
            vector<int> p_gen;//最终选择的子代

            for(int j=0;j<graph_node_num;j++){//交叉产生两个后代
                if(rand()/(RAND_MAX+1.0) >0.5) p1_gen.push_back(p_best_array[i][j]);
                else p1_gen.push_back(p_best_array[g_best_id][j]);
                if(rand()/(RAND_MAX+1.0) >0.5) p2_gen.push_back(p_best_array[i][j]);
                else p2_gen.push_back(p_best_array[g_best_id][j]);
            }//ENDFOR

            if( rand()/(RAND_MAX+1.0) < 0.5){//随机选择其中一个基因作为子代
                for(int j=0;j<graph_node_num;j++){
                    p_gen.push_back(p1_gen[j]);
                }
                p1_gen.clear();
            }
            else {
                for(int j=0;j<graph_node_num;j++){
                    p_gen.push_back(p2_gen[j]);
                }
                p2_gen.clear();
            }
            ...
            // QBPSO step 6 更新粒子的新的位置

            //计算当前粒子到平均最好位置的海明威距离（亦即两个串间不同位的个数）
                              
            //根据变异概率，变异当前产生的后代粒子
            for(int j=0;j<graph_node_num;j++){
                //求当前位的变异概率
                int distance = 0;
                if(aver_best[j] != Swarm[i][j]) distance=1;
                double Pm = QBPSO_alpha * distance*(log(1/(rand()/(RAND_MAX+1.0)))); 
                printf("当前粒子的变异概率：%f\n",Pm);
                if(rand()/(RAND_MAX+1.0) < Pm){//已一定的概率变异
                    if(p_gen[j] == 1) p_gen[j]=0;
                    else p_gen[j]=1;
                }
            }
