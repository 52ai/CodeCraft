...
vector<int> all_net_to_consume;
vector<int> all_net_to_consume_value;

int least_consume_bandwidth;
...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num, char * filename){
    ...
    for (int i=0;i<line_num;i++){
        ...
        switch(space_line_count){
            ...
            case 0 :{
                ...
                //设定可设为服务器直连消费节点的网络节点的最小消费量

                if(graph_node_num < 200) {least_consume_bandwidth=60;}
                else if(graph_node_num > 200 && graph_node_num < 500) {least_consume_bandwidth=60;}
                else {least_consume_bandwidth=60;}
            ...
            case 3 :{
                ...
                all_net_to_consume.push_back(net_node);
                all_net_to_consume_value.push_back(demand_bandwidth);

                demand_flow += demand_bandwidth;

                /*
                if(demand_bandwidth > least_consume_bandwidth){//如果当前消费节点需求大于100，则选择直连.（大于20相对较好）
    ...
    **/
    ...
    double w;
    int r1,r2;//粒子群算法的惯性系数
    int max_repeat_cnt;//记录最优解最大重复次数
    double pc,pm;//交叉和变异的概率
    int time_limit;
    //根据图的规模设置粒子群小法惯性系数w
    if(graph_node_num < 200) {w=0.9;r1=1;r2=2;max_repeat_cnt=50;pc=0.8;pm=0.1;time_limit=80;}
    else if(graph_node_num > 200 && graph_node_num < 500) {w=0.5;r1=1;r2=2;max_repeat_cnt=50;pc=0.8;pm=0.2;time_limit=80;}
    else {w=0.005;r1=1;r2=3;max_repeat_cnt=20;pc=0.8;pm=0.5;time_limit=75;}

    /***
    关于粒子群的初始化：
    第一，所有与消费节点相连的都不作为服务器的候选点。（不可行）
    第二，服务器的个数要小于消费节点的个数（可行）
    第三，服务器的个数要大于（消费节点需求流量）/所有网络节点出度流量的平均值）

    ****/
    /**

    其他的一些思考：按消费节点的需求，需求大于某一值的直接放置服务器

    ****/
    //对直连消费带宽排序
    printf("全部直连消费节点的个数：%lu\n",all_net_to_consume.size());
    for(int i=0;i<all_net_to_consume.size();i++){
        for(int j=i+1;j<all_net_to_consume.size();j++){
            if(all_net_to_consume_value[i] < all_net_to_consume_value[j]){//交换
                int temp_value = all_net_to_consume_value[i];
                int temp_node = all_net_to_consume[i];

                all_net_to_consume_value[i]=all_net_to_consume_value[j];
                all_net_to_consume[i]=all_net_to_consume[j];
                all_net_to_consume_value[j]=temp_value;
                all_net_to_consume[j]=temp_node;

            }
        }
    }
    /*
    for(int i=0;i<all_net_to_consume_value.size();i++){
        printf("排序啦(%d,%d)",all_net_to_consume[i],all_net_to_consume_value[i]);
    }*/

    //选取合适的直连网络节点
    int top_n = 2*consume_node_num/3;

    for(int i=0;i<top_n;i++){
        net_to_consume.push_back(all_net_to_consume[i]);
    }


    printf("消费需求大于给定下限的消费节点个数：%lu\n",net_to_consume.size());
    ...
    for(int i=0;i<graph_node_num;i++){
        ...
        Swarm[Swarm_Size-2][i]=0;
    }

    for(int i=0;i<all_net_to_consume.size();i++){

        Swarm[Swarm_Size-1][all_net_to_consume[i]]=1;
    }
    
    for(int i=0;i<net_to_consume.size();i++){

        Swarm[Swarm_Size-2][net_to_consume[i]]=1;
    }
    ...
    while(((finish_time - start_time)/CLOCKS_PER_SEC < time_limit) && (optimal_repeat_cnt < max_repeat_cnt)){//每一次循环进行一次粒子的更新并计算适应度
        printf("第%d代粒子群:\n",++iter_times);
        ...
        for(int i=0;i<Swarm_Size;i++){
            for(int j=0;j<Swarm[i].size();j++){//访问每一个粒子的每一个二进制位，对其进行更新变异
                curr_v =w*Velocity[i][j]+r1*rand()*(p_best_array[i][j]-Swarm[i][j])+r2*rand()*(p_best_array[g_best_id][j]-Swarm[i][j]);
                //printf("当前粒子的速度:%f\n",curr_v );
                //大图，w=0.2,小图，w=0.7,中图，w=0.7
        ...
        if((rand() /(RAND_MAX+1.0)) < pc) {//以一定的概率的概率进行变异
        ...
        if(rand()/(RAND_MAX+1.0) < pm){//以一定的概率的概率进行变异
    ...
    //=======================================================================================//
    printf("粒子群算法总共运行时间：%f s,w系数为:%f\n",(finish_time - start_time)/CLOCKS_PER_SEC, w);
    //找到当前最优解
    MCMF* mcmf = new MCMF();
    mcmf->init(n,m,s,t,graph_node_num);
    //复制图
